...
/* My memset() implementation -- to support proper hashing even when IP protection is enabled */
int ii __attribute__((section(".ipe_vars")));
unsigned char *temp __attribute__((section(".ipe_vars")));
__attribute__((section(".ipe_code"))) void myMemSet(void *ptr, int value, int size);
...
/* My memcpy() implementation -- to support proper hashing even when IP protection is enabled */
char *csrc __attribute__((section(".ipe_vars")));
char *cdest __attribute__((section(".ipe_vars")));
__attribute__((section(".ipe_code"))) void myMemCpy(void *dest, void *src, size_t n);
...
int _i __attribute__((section(".ipe_vars")));
#if defined(SWAP_BYTES)
#define bsw_32(p,n) \
    { _i = (n); while(_i--) ((uint32_t*)p)[_i] = bswap_32(((uint32_t*)p)[_i]); }
#else
#define bsw_32(p,n)
#endif
...
const uint32_t k256[64] __attribute__((section(".ipe_const")))=
...
uint32_t *p __attribute__((section(".ipe_vars")));
//uint32_t v0 __attribute__((section(".ipe_vars")));
//uint32_t v1 __attribute__((section(".ipe_vars")));
//uint32_t v2 __attribute__((section(".ipe_vars")));
//uint32_t v3 __attribute__((section(".ipe_vars")));
//uint32_t v4 __attribute__((section(".ipe_vars")));
//uint32_t v5 __attribute__((section(".ipe_vars")));
//uint32_t v6 __attribute__((section(".ipe_vars")));
//uint32_t v7 __attribute__((section(".ipe_vars")));
uint32_t j __attribute__((section(".ipe_vars")));
uint32_t v[8] __attribute__((section(".ipe_vars")));
VOID_RETURN sha256_compile(sha256_ctx ctx[1])
{
    ...
    myMemCpy(v, ctx->hash, sizeof(ctx->hash));
...
uint32_t pos __attribute__((section(".ipe_vars")));
unsigned char *sp __attribute__((section(".ipe_vars")));  //was const type previously, changed it to vars so that the assignment at line 302 is possible.
unsigned char *w __attribute__((section(".ipe_vars")));
uint32_t space __attribute__((section(".ipe_vars")));
VOID_RETURN sha256_hash(const unsigned char data[], unsigned long len, sha256_ctx ctx[1])
{   pos = (uint32_t)((ctx->count[0] >> 3) & SHA256_MASK);
    sp = data;
    w = (unsigned char*)ctx->wbuf;
#if SHA2_BITS == 1
    ...
    {   space = SHA256_BLOCK_SIZE - pos;
        ...
        myMemCpy(w + pos, sp, (len + 7 * SHA2_BITS) >> 3);
    }
}

/* SHA256 Final padding and digest calculation  */

//uint8_t hash_test[32] __attribute__((section(".persistent"))); // the output of SHA256 will be stored here
uint32_t i __attribute__((section(".ipe_vars")));
uint32_t m1 __attribute__((section(".ipe_vars")));
__attribute__((section(".ipe_code"))) static void sha_end1(unsigned char hval[], sha256_ctx ctx[1], const unsigned int hlen);
static void sha_end1(unsigned char hval[], sha256_ctx ctx[1], const unsigned int hlen)
{   i = (uint32_t)((ctx->count[0] >> 3) & SHA256_MASK);
    ...
    for(i = 0; i < hlen; ++i)
    {
        ...
        //hash_test[i] = ((ctx->hash[i >> 2] >> (8 * (~i & 3))) & 0xff);
    }
}
...
const uint32_t i224[8] __attribute__((section(".ipe_const"))) =
{
...
VOID_RETURN sha224_begin(sha224_ctx ctx[1])
{
    ...
    myMemSet(ctx, 0, sizeof(sha224_ctx));
    //memcpy(ctx->hash, i224, sizeof(ctx->hash));
    myMemCpy(ctx->hash, i224, sizeof(ctx->hash));
}
...
sha224_ctx cx224[1] __attribute__((section(".ipe_vars")));  //this was originally cx and defined inside the function sha224, changed it's name to avoid conflict with already existing variable cx.
VOID_RETURN sha224(unsigned char hval[], const unsigned char data[], unsigned long len)
{
    sha224_begin(cx224);
    sha224_hash(data, len, cx224);
    sha_end1(hval, cx224, SHA224_DIGEST_SIZE);
}
...
const uint32_t i256[8] __attribute__((section(".ipe_const"))) =
{
...
VOID_RETURN sha256_begin(sha256_ctx ctx[1])
{
    ...
    myMemSet(ctx, 0, sizeof(sha256_ctx));
    //memcpy(ctx->hash, i256, sizeof(ctx->hash));
    myMemCpy(ctx->hash, i256, sizeof(ctx->hash));
}
...
__attribute__((section(".ipe_code"))) INT_RETURN sha2_begin(unsigned long len, sha2_ctx ctx[1]);
INT_RETURN sha2_begin(unsigned long len, sha2_ctx ctx[1])
{
    switch(len)
    {
        ...
        case  28:   CTX_256(ctx)->count[0] = CTX_256(ctx)->count[1] = 0;
                    ...
                    myMemCpy(CTX_256(ctx)->hash, i224, 32);
...
__attribute__((section(".ipe_code"))) VOID_RETURN sha2_hash(const unsigned char data[], unsigned long len, sha2_ctx ctx[1]);
...
__attribute__((section(".ipe_code"))) VOID_RETURN sha2_end(unsigned char hval[], sha2_ctx ctx[1]);
...
sha2_ctx cttx[1] __attribute__((section(".ipe_vars"))); //this was originally cx and defined inside the function sha2, changed it's name to avoid conflict with already existing variable cx.
__attribute__((section(".ipe_code"))) INT_RETURN sha2(unsigned char hval[], unsigned long size, const unsigned char data[], unsigned long len);
INT_RETURN sha2(unsigned char hval[], unsigned long size,
                                const unsigned char data[], unsigned long len)
{
    if(sha2_begin(size, cttx) == EXIT_SUCCESS)
    {
        sha2_hash(data, len, cttx); sha2_end(hval, cttx); return EXIT_SUCCESS;
    }
    else
        return EXIT_FAILURE;
}
